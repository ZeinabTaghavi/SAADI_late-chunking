...
for retriever_name in "${retriever_array[@]}"; do
  ...
  while IFS= read -r manifest_path; do
    all_run_dirs+=("$(dirname "${manifest_path}")")
  done < <(find "${retriever_root}" -type f -name 'run_manifest.json' | sort)
done

if [[ "${#all_run_dirs[@]}" -eq 0 ]]; then
  printf 'No QuALITY run manifests were found under %s/%s for retrievers: %s\n' \
    "${RUN_ROOT}" \
    "${DATASET_NAME}" \
    "${RETRIEVERS}"
  exit 1
...
printf 'Running batch retrieval evaluation for QuALITY:\n'
...
printf 'Finished QuALITY retrieval evaluation batch. failed_runs=%s total_runs=%s\n' \
  "${failed_runs}" \
  "${#all_run_dirs[@]}"
