...
DATASET_NAME="loogle"
CONFIG_PATH="${CONFIG_PATH:-configs/experiments/loogle_retrieval_ablation.yaml}"
...
printf 'Evaluating LooGLE c500 retrieval runs:\n\n'
eval_index=0
for retriever_name in "${retriever_array[@]}"; do
  for chunk_overlap in "${overlap_array[@]}"; do
...
printf 'Finished LooGLE c500 rerun. failed_runs=%s total_steps=%s\n' \
  "${failed_runs}" \
