...
if [[ "${#all_run_dirs[@]}" -eq 0 ]]; then
  printf 'No LooGLE run manifests were found under %s/%s for retrievers: %s\n' \
    "${RUN_ROOT}" \
    "${DATASET_NAME}" \
    "${RETRIEVERS}"
  exit 1
...
printf 'Running batch retrieval evaluation for LooGLE:\n'
...
printf 'Finished LooGLE retrieval evaluation batch. failed_runs=%s total_runs=%s\n' \
  "${failed_runs}" \
  "${#all_run_dirs[@]}"
