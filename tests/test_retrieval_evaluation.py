...
def test_evaluate_run_writes_compact_artifacts(tmp_path):
    ...
    assert metrics_summary["ranking_metrics_by_view"]["gold"]["recall@5"] == pytest.approx(0.5)
    assert metrics_summary["ranking_metrics_by_view"]["gold"]["mrr@5"] == pytest.approx(0.5)
    assert metrics_summary["ranking_metrics_by_view"]["silver_loose"]["mrr@5"] == pytest.approx(0.5)
    assert metrics_summary["ranking_metrics_by_view"]["union_loose"]["recall@5"] == pytest.approx(0.75)
    assert metrics_summary["hit_rate_metrics_by_view"]["gold_hit"]["hit_rate@10"] == 1.0
    assert metrics_summary["hit_rate_metrics_by_view"]["gold_hit"]["hit_rate@5"] == 1.0
    assert metrics_summary["hit_rate_metrics_by_view"]["silver_strict_hit"]["hit_rate@5"] == 1.0
    assert metrics_summary["hit_rate_metrics_by_view"]["strict_union_hit"]["hit_rate@5"] == 1.0
    assert leaderboard_row["gold_ndcg@10"] == metrics_summary["ranking_metrics_by_view"]["gold"]["ndcg@10"]
    ...
    assert per_query_rows[0]["gold_recall@5"] == pytest.approx(0.5)
...
def test_evaluate_run_reports_null_metrics_when_only_grouped_silver_exists(tmp_path):
    ...
    assert metrics_summary["ranking_metrics_by_view"]["gold"]["recall@5"] is None
    assert metrics_summary["hit_rate_metrics_by_view"]["silver_strict_hit"]["hit_rate@5"] == 1.0
    assert metrics_summary["hit_rate_metrics_by_view"]["strict_union_hit"]["hit_rate@5"] == 1.0
...
def test_evaluate_run_generates_quality_labels_from_answer_choice_text(tmp_path):
    run_dir = tmp_path / "late_chunk_runs" / "quality" / "jina" / "c300_o0"
    _write_json(
        run_dir / "run_manifest.json",
        {
        ...
        [
            {
                ...
                "question": "Who solved the puzzle?",
                "answers": ["Mira Solved It"],
                "retrieval_spans": [],
                "choices": ["Mira Solved It", "No one did", "The mayor did", "It was unknown"],
            }
        ],
    )
    _write_jsonl(
        run_dir / "chunking" / "article:1" / "chunks.jsonl",
        [
            {
                "doc_id": "article:1",
                "chunk_id": "q1",
                "chunk_index": 0,
                "raw_text": "The story opens with a long introduction.",
            },
            {
                "doc_id": "article:1",
                "chunk_id": "q2",
                "chunk_index": 1,
                "raw_text": "In the final scene, Mira solved it and explained the trick.",
            },
        ],
    )
    _write_jsonl(
